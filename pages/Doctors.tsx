...
const NextArrow = (props: any) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`absolute -right-4 top-1/2 -translate-y-1/2 z-10 p-3 bg-white rounded-full shadow-lg border border-slate-100 hover:bg-teal-50 text-slate-600 hover:text-teal-600 transition-all group ${
...
const PrevArrow = (props: any) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`absolute -left-4 top-1/2 -translate-y-1/2 z-10 p-3 bg-white rounded-full shadow-lg border border-slate-100 hover:bg-teal-50 text-slate-600 hover:text-teal-600 transition-all group ${
        className?.includes("slick-disabled")
          ? "opacity-0 pointer-events-none"
          : "opacity-100"
      }`}
      ...
      <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-3">
        ...
        <div className="flex items-center gap-2 text-[10px] font-medium text-slate-500">
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-teal-500"></span> Available
          </span>
        </div>
      </div>
...
const Doctors: React.FC = () => {
  ...
  // Automatically rotate the doctors list every 3 seconds when appropriate
  const autoRotateActive =
    !searchTerm &&
    !specialtyFilter &&
    !selectedDoctor &&
    !loading &&
    !isHovering;
  useEffect(() => {
    if (!autoRotateActive || doctors.length <= 1) return;
  ...
  }, [autoRotateActive, doctors.length]);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            ...
            <p className="text-slate-600 text-lg">
              World-class doctors committed to your health.
            </p>
          </div>
          <Link
            to="/appointments"
            className="hidden md:inline-flex bg-slate-900 text-white px-6 py-3 rounded-lg font-medium hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20"
          >
            Book Appointment
          </Link>
        </div>
                  ...
                  >
                    <div className="p-6 pb-0 flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 truncate mb-1 group-hover:text-teal-600 transition-colors">
                          {doctor.name}
                        </h3>

...
const API_URL = '/api';
const REQUEST_TIMEOUT_MS = 2000; // Reduced timeout for faster fallback
...
export const authApi = {
  ...
  register: async (name: string, email: string, password: string): Promise<{ user: User; token: string }> => {
    try {
        ...
        // Mock registration for offline/demo mode
        console.warn('Registration failed, using offline fallback');
        const mockUser = {
